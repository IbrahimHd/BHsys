        ...
        function widgetControls($window, $document, $compile, $templateRequest, $timeout) {
            return {
              //priority:1,
                ...
                replace: true,
                transclude: true,
                templateUrl: './home/vizTplTransclude.html',
                scope:{
                    widgetControls: '='
                },
                link: function (scope, element, attrs) {
                    ...
                    //var param = angular.fromJson(scope.widgetControls);
                    scope.chartId = 'map';// scope.widgetControls.id; //attrs.id;
                    scope.type = scope.widgetControls.type;
                    scope.chart = scope.widgetControls.chart;
                    scope.chartTitle = scope.widgetControls.title;
                    scope.iconClass = scope.widgetControls.iconClass;
                    ...
                    function setChartSize() {
                        ...
                        if (scope.type === 'geoChoropleth') {  // << this is a member of the core dc.js (not dc.leaflet)
                            ...
                            scope.chart.projection(d3.geo.mercator()
                                        .scale(10000)
                                        .center(scope.isExpanded ? [37.6, 33.5] : [37.6 - dif, 33.6])
                                        )
                        }
                    ...
                    $document.on('keyup', function (e) {
                        if (e.keyCode === 27 && scope.isExpanded) {
                    ...
                    scope.resetChart = function (chart) {
                        chart.filterAll(); chart.redrawGroup();
                    }
                }
            }
        }
    
        vizCtrl.$inject = ['$scope', 'dc_', 'dataService', 'debugMode'];
    
        function vizCtrl($scope, dc_, dataService, debugMode) {
            ...
            var all = xf.groupAll();
            var groupName = 'A';
            var rowStackChart = dc_.rowChart("#rowStackChart"),
                ...
                dc.events.trigger(function () {
                    ...
                    dc_.chartRegistry.list(groupName).forEach(chart => {
                      ...
                      //console.log(chart.dimension().top(1)[0], chart.group().top(1)[0].key);
                        if (chart.filters().length > 0) {
                            isFiltered = true;
                            if (chart.filters()[0] instanceof Array) {
                                htmlCode += '<i class="fa fa-filter"></i> <span class="text-bg">Date</span>=<span class="text-md">[' + dateFormat(chart.filters()[0][0]) + ", " + dateFormat(chart.filters()[0][1]) + "]</span> ";
                            }
                            else if (chart.filters()[0] instanceof Date) {
                                htmlCode += '<i class="fa fa-filter"></i> <span class="text-bg">Date</span>=<span class="text-md">' + dateFormat(chart.filters()[0]) + "</span> ";
                            }
                            else {
                                htmlCode += '<i class="fa fa-filter"></i> <span class="text-bg">' + chart.anchorName() + '</span>=<span class="text-md">' + chart.filters() + "</span>";
                            }
                        }
                    });
