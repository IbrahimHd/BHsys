...
namespace BH.Auth
{
    ...
    public static class SecurityManager
    {
        ...
        public static string GenerateToken(string username, string password, string ip, string userAgent, long ticks)
        {
            ...
            Debug.Write("GenerateToken: " + Convert.ToBase64String(Encoding.UTF8.GetBytes(string.Join(":", hashLeft, hashRight))));
        ...
        public static string GetHashedPassword(string password)
        {
            ...
            using (HMAC hmac = HMACSHA256.Create(_alg))
            {
                ...
                Debug.WriteLine("Server key: " + Convert.ToBase64String(hmac.Hash));
        ...
        public static bool IsTokenValid(string token, string ip, string userAgent)
        {
            ...
            try
            {
                ...
                if (parts.Length == 3)
                {
                    ...
                    if (!expired)
                    {
                        ...
                        //
                        Debug.WriteLine("---IsTokenValid---");

                        string password = UserAccountUtility.getUserPassword(username);

                        Debug.WriteLine("Server Ticks: " + ticks);
                        Debug.WriteLine("Sever IP: " + ip);
                        
                        // Hash the message with the key to generate a token.
                        string computedToken = GenerateToken(username, password, ip, userAgent, ticks);
                        Debug.WriteLine("computedToken: " + computedToken);
            ...
            {
                var errMsg= "Error: " + ex.GetBaseException().Message;
                return false; //errMsg;
            }
            Debug.Write("IsTokenValid " + result);
            return result;
        }
    }
}