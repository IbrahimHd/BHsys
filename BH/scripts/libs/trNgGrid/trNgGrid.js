...
(function (TrNgGrid) {
    ...
    angular.module("trNgGrid", [])
        ...
        .run(function () {
        TrNgGrid.tableCssClass = "tr-ng-grid table table-bordered table-hover table-striped"; // <<< "tr-ng-grid table table-bordered table-hover";
        ...
        TrNgGrid.columnSortInactiveCssClass = "tr-ng-sort-inactive text-muted fa fa-chevron-down"; // <<< "tr-ng-sort-inactive text-muted glyphicon glyphicon-chevron-down";
        TrNgGrid.columnSortReverseOrderCssClass = "tr-ng-sort-order-reverse fa fa-chevron-down"; // <<< "tr-ng-sort-order-reverse glyphicon glyphicon-chevron-down";
        TrNgGrid.columnSortNormalOrderCssClass = "tr-ng-sort-order-normal fa fa-chevron-up"; // <<< "tr-ng-sort-order-normal glyphicon glyphicon-chevron-up";
        TrNgGrid.rowSelectedCssClass = "active";
        TrNgGrid.footerCssClass = "tr-ng-grid-footer form-inline";
    })
        .run(function () {
        TrNgGrid.defaultColumnOptions.displayAlign = 'right'; // <<< 'left';
...
// <<<      + '     <span ng-hide="totalItemsCount">{{\'No items to display\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}}</span>'
            + '     <span ng-hide="totalItemsCount">{{\'لم يتم العثور على شيء\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}}</span>'
            + '     <span ng-show="totalItemsCount">'
// <<<      + '       {{startItemIndex+1}} - {{endItemIndex+1}} {{\'displayed\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}}'
            + '       {{\'عرض\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}} {{startItemIndex+1}} - {{endItemIndex+1}} '
// <<<      + '       <span>, {{totalItemsCount}} {{\'in total\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}}</span>'
            + '       <span>{{\'من أصل\'|' + TrNgGrid.translateFilter + ':gridOptions.locale}} {{totalItemsCount}} </span>'
            + '     </span > '
            + '   </li>'
            + ' </ul>'
            + '</span>');
